...
//        Mage::getSingleton('customer/session')->unsQuoteCreatedThroughUpload();
    ...
    public function deleteAction()
    {
        ...
        if ($id) {
        $currentProductCaluculatedPrice = Mage::getSingleton('core/session')->getsoftwareProdPrice();
        $softProd = Mage::getSingleton('core/session')->getsoftwareProdPrice();

        $tot = $softProd - $currentProductCaluculatedPrice;
        Mage::getSingleton('core/session')->setsoftwareProdPrice($tot);
            try {
    ...
    public function checkSoftwareProdAction(){
        if(isset($_GET['id'])){
            $quantityUpdateAllowed = 0;
            $rawData = $_GET['id'];
            $rawDataToArray = explode('[',$rawData);
            $content = str_replace(array(']'),'',$rawDataToArray[1]);
            $id = trim($content);
            $cart = $this->_getCart();
            $quoteItem = $cart->getQuote()->getItemById($id);
            $proId = $quoteItem->getProduct()->getId();
            ...
            if(strtolower($prodType)=='maintenance product'){
                $quantityUpdateAllowed = 1;
                echo $quantityUpdateAllowed;
            }
        }
    }
}
