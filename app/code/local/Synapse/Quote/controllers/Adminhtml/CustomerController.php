...
class Synapse_Quote_Adminhtml_CustomerController extends Mage_Adminhtml_CustomerController
{
	...
	$data = $this->getRequest()->getPost();	
		...
		if($data['account']['is_dealer']){			
			$customer_old_data = Mage::getModel('customer/customer')->load($customer->getId());		
			$old_group_id = $customer_old_data->getGroupId();
			
			//get all the sub accounts of this dealer account
												
			$dealer_sub_accounts = Mage::getModel('customer/customer')->getCollection()->addAttributeToFilter('master_customer',$customer->getId());
			foreach($dealer_sub_accounts as $dealersubaccount){
				$dealer_subaccount_groupid = $dealersubaccount->getGroupId();
				if($dealer_subaccount_groupid == $old_group_id)
					$dealersubaccount->setGroupId($data['account']['group_id']);
				 	
				try{
				    $dealersubaccount->save();	
				}catch (Exception $e) {
