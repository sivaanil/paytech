...
$k(document).ready(function(){
	if(!$k(".header-cart").is(':visible')){
		$k(".header-content").css('border-right','none');
	}
});

$k(document).ready(function(){
	if($k("#products-list").is(':visible')){
		$k("#products-list li").each(function(){
			if(!$k(this).children(".list-right").is(':visible'))
			{
				$k(this).children(".list-center").css('border-right','none');
			}
		});		
	}
});
...
function submitQuote(){
	if(jQuery("#service_number").val()!=""){
		jQuery("#my-quote").submit();
	}else{
...
jQuery(document).ready(function(){
	jQuery("#billing-address-select option:last").hide();
	jQuery("#shipping-address-select option:last").hide();
});
